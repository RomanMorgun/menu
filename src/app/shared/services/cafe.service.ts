...
export class CafeService {
    ...
    getAllCafes(params: object = {}): Observable<any> {
        return this.requestService.get(`${COMMON_URL.cafe.getAll}`, {...params}).pipe(
            tap((result) => {
                this.setCafes(result.data && result.data.cafes);
            }, (error) => {
                console.log(error);
            })
        );
    ...
    getCafesByCoords(coords): Observable<any> {
        return this.requestService.getForCafes(`${COMMON_URL.cafe.getAll}`, coords).pipe(
            tap((result) => {
                    this.setCafes(result && result.data && result.data.cafes);
                    console.log(result);
                },
                (error) => {
                    console.log(error);
